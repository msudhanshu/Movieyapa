 ...
 */ 
public class UIQuestPanel: UIGamePanel {

//	public Text questText;
	private List<UIQuestItem> uiQuestItems;
...
//		content.transform.position = showPosition;	
	...
	virtual protected void PopulateData() {
		if(UserQuest.activeQuests==null || UserQuest.activeQuests.Count <= 0)
			return;
		if(uiQuestItems==null)
			uiQuestItems = new List<UIQuestItem>();
		ClearAllItems();
		
		int i = 0;
		foreach(UserQuest quest in UserQuest.activeQuests) {
			Transform go = CreateItemPrefab(questItemPrefab,i);
			UIQuestItem panel = go.GetComponent<UIQuestItem>();
			panel.InitialiseWithData(quest);
			uiQuestItems.Add (panel);
			i++;
		}
		ResizeGridLayout(i);
	}
	...
	private void ClearAllItems() {
		if(uiQuestItems!=null) {
			foreach(UIQuestItem item in uiQuestItems) {
