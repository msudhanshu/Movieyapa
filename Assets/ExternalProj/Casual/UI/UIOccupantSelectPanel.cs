...
public class UIOccupantSelectPanel : MonoBehaviour
{
	...
	public void UpdateOccupantStatus() {
		...
		if (type.level == 0) {
			levelLabel.text = "";
		} else if (type.level <= ResourceManager.GetInstance().Level) {
			levelLabel.text = "[000000]Level " + type.level;
		} else {
			levelLabel.text = "[ff0000]Requires Level " + type.level;
		}
		if (OccupantManager.GetInstance().CanRecruitOccupant(type.id) && BuildingManager3D.ActiveBuilding.CanFitOccupant(type.occupantSize)) {
			...
			recruitButton.InitWithActivityType(DoActivity, StateTransitionType.RECRUIT, type.id);
		} else {
	...
	public void DoActivity(string type, string supportingId) {
		if (BuildingManager3D.ActiveBuilding.StartTransition (type, System.DateTime.Now, supportingId)) {
