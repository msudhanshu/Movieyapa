...
public class AttackResultsHandler : MonoBehaviour
{
	 ...
	 */ 
	public void CustomReward(StateTransition activity) {
		...
		if (type is AttackActivityData) {
			...
			// Add rewards
			ResourceManager.GetInstance().AddSilver(resourcesRewarded);
			ResourceManager.GetInstance().AddGold(goldRewarded);
			...
			if (panel != null) {
				...
				PopupManager.GetInstance().ShowPanel(PanelType.BATTLE_RESULTS);
			}
		}
	}
}
