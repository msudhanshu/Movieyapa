 ...
 */
	...
	{
		return Create(Vector2.zero, Vector3.one);
	}
    public GameObject Create(Vector2 uvOffset, Vector2 uvScale)
    {
        ...
        Mesh m = null; //  (Mesh)AssetDatabase.LoadAssetAtPath("Assets/" + planeAssetName,typeof(Mesh));
        ...
        if (m == null)
        {
            ...
            for (float y = 0.0f; y < vCount2; y++)
            {
                for (float x = 0.0f; x < hCount2; x++)
                {
					...
					uvs[index++] = (new Vector2(x*uvFactorX * uvScale.x, y*uvFactorY * uvScale.y)) + uvOffset;
                }
            }
