...
public class MazeMapGeneratorAlgo : IMazeMapGeneratorAlgo {
...
*/
	...
	private MazeMap GenerateMaze()
	{

		FillMazeBlock(MazeBlockType.FREE);
	...
	private void CreateMaze()
	{
		...
		while (_tiletoTry.Count > 0)
		{
			...
			mazeMap.mazeGrid[(int)CurrentTile.x, (int)CurrentTile.y] = MazeBlockType.BLOCK1;
	...
	private List<Vector2> GetValidNeighbors(Vector2 centerTile)
	{
		...
		foreach (var offset in offsets)
		{
			...
			if (toCheck.x % 2 == 1 || toCheck.y % 2 == 1)
			{
				...
				if (mazeMap.mazeGrid[(int)toCheck.x, (int)toCheck.y] == MazeBlockType.FREE && HasThreeWallsIntact(toCheck))
				{
					//add the neighbor
	...
	private bool HasThreeWallsIntact(Vector2 Vector2ToCheck)
	{
		...
		foreach (var offset in offsets)
		{
			...
			if (IsInside(neighborToCheck)  && mazeMap.mazeGrid[(int)neighborToCheck.x, (int)neighborToCheck.y] == MazeBlockType.FREE)
			{
				intactWallCounter++;
			}
		}
