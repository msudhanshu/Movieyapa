Shader "MyShader/TextureCoordinates/Mandelbrot" {
	Properties {
		_MaxItere("_MaxIter", Float) = 29
		}
	...
	SubShader {
		Pass {
			...
			float4 frag(v2f_img i) : COLOR {
				...
				float iteration = 0.0;
				const float _MaxIter = 29;//_MaxItere;
				...
				float val = fmod((iteration/_MaxItere)+_Time.x,1.0);
				float4 color;
