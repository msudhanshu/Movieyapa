Shader "MyShader/TextureCoordinates/Effect Sliding" {
	Properties
	{
	...
	SubShader {
		Pass {
			...
			float4 frag(fragmentInput i) : COLOR {
				...
				if (fmod(_SliceCount*i.texcoord0.x,2.0)<1.0 && fmod(_SliceCount*i.texcoord0.y,2.0)<1.0) 
				{
				
				i.texcoord0 = 	i.texcoord0 + float2(_SlideAmount,_SlideAmount);
