...
namespace SgUnity
{
    ...
    public class QuestionAnswerManager : Manager<QuestionAnswerManager>
    {
        ...
        public void OnQuestionAnswered(QuestionModel q, int score)
        {
            if (score > 0)
                questionAnswered.OnCorrectAnswer(q);
            else
                questionAnswered.OnWrongAnswer(q);
        }
        ...
        {
            QuestionProvider qProvider = new QuestionProvider();
            int qid = qProvider.getQuestion(1, CarrierManager.CarrierReplayType.NEW, 0);
            SgUnity.SgRemoteAction.GetQuestionByIdAction(qid, new GetQuestionCallback());
...
//                else
    ...
    public class GetQuestionCallback : SgUnity.ServerNotifier
    {
        ...
        public virtual void OnSuccess(GameResponse response)
        {
            QuestionModel q = JsonUtility.FromJson<QuestionModel>(response.response);
            // QuestionModel q = JsonConvert.DeserializeObject<QuestionModel>(response.response);
            QuestionAnswerManager.GetInstance().SetQuestionToUI(q);
        }
